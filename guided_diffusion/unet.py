...
class QKVAttentionLegacy(nn.Module):
    ...
    def forward(self, qkv):
        ...
        # q = th.softmax(q.float()*scale, dim=-1).type(q.dtype)
        # k = th.softmax(k.float()*scale, dim=-1).type(k.dtype)
        # weight = th.bmm(k.transpose(1, 2), v)
        # a = th.bmm(q, weight)
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.num_classes is not None:
            # print(x.shape, y.shape, self.num_classes)
        ...
        try:
            ...
            # change the self.out module to fp16
            # self.out.apply(convert_module_to_f16)
            return self.out(h.type(th.float32)).type(x.dtype)
